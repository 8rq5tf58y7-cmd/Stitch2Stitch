...
class ImageAligner:
    ...
    def _estimate_similarity_transform(
        ...
        If allow_scale=False, directly estimates rigid transform (rotation + translation only).
        """
        if len(pts1) < 2:
            return None

        # If scaling is disabled, use rigid transform directly
        if not self.allow_scale:
            return self._estimate_rigid_transform(pts1, pts2)
        ...
        if not self.allow_scale:
            ...
            if abs(scale - 1.0) > 0.05:
                logger.info(f"Scale {scale:.3f} detected, re-estimating with rigid transform (rotation + translation only)")
                # Re-estimate with rigid transform (rotation + translation, no scale)
                return self._estimate_rigid_transform(pts1, pts2)
        else:
    ...
    def _estimate_rigid_transform(
        ...
        """
        Estimate rigid transform (rotation + translation, NO scale).

        Uses Umeyama algorithm for closed-form solution with RANSAC for robustness.
        Transform has 3 DOF: translation (tx, ty), rotation (θ)
        Matrix form: [[cos(θ), -sin(θ), tx],
                      [sin(θ),  cos(θ), ty],
                      [0,       0,       1]]
        """
        if len(pts1) < 3:
        ...
        for _ in range(n_iterations):
            ...
            if len(pts1) < 2:
                break
            idx = np.random.choice(len(pts1), size=min(2, len(pts1)), replace=False)
            sample_pts1 = pts1[idx]
            sample_pts2 = pts2[idx]

            # Estimate rigid transform from sample
            # Center points
            centroid1 = np.mean(sample_pts1, axis=0)
            centroid2 = np.mean(sample_pts2, axis=0)
            centered1 = sample_pts1 - centroid1
            centered2 = sample_pts2 - centroid2
            ...
            pts2_est = (transform @ pts1_h.T).T[:, :2]
            errors = np.linalg.norm(pts2 - pts2_est, axis=1)
        ...
        pts1_h = np.column_stack([pts1, np.ones(len(pts1))])
        pts2_est = (best_transform @ pts1_h.T).T[:, :2]
        errors = np.linalg.norm(pts2 - pts2_est, axis=1)
        ...
        centroid1 = np.mean(pts1_inliers, axis=0)
        centroid2 = np.mean(pts2_inliers, axis=0)
        centered1 = pts1_inliers - centroid1
        centered2 = pts2_inliers - centroid2
        ...
        refined_transform = np.eye(3, dtype=np.float64)
